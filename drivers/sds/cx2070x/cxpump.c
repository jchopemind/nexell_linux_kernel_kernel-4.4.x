...
typedef struct CX_CODEC_ROM{
    ...
    unsigned int               LoaderAddr;
    unsigned int               LoaderLen;
    unsigned int               CtlAddr;
    unsigned int               CtlLen;
    unsigned int               SpxAddr;
    unsigned int               SpxLen;
...
typedef struct CX_CODEC_ROM_APPENDED{
    unsigned int               TuningAddr;
    unsigned int               TuningLen;
...
unsigned int        g_cbMaxWriteBufSize        = 0;
...
*/
unsigned int ByteOrderSwapULONG(unsigned int i)
{
...
*/
unsigned int ToBigEndiaULONG(unsigned int i)
{
...
*/
unsigned int FromBigEndiaULONG(unsigned int i)
{
#if __BYTE_ORDER == __LITTLE_ENDIAN
    return ByteOrderSwapULONG(i);
#else
...
*/
int ChannelI2cBulkWrite( unsigned int NumOfBytes, unsigned char *pData)
{
    ...
    unsigned int  BytesToProcess       = 0;
    unsigned short backup               = 0;
    const unsigned int cbAddressBytes  = 2;
    const unsigned int cbMaxDataLen    = g_cbMaxWriteBufSize-cbAddressBytes;
...
*    Otherwise, return ERRON_* error code. 
*/
int CxWriteMemory(unsigned int dwAddr, unsigned int NumOfBytes, unsigned char * pData, int MemType )
{
    int ErrNo           = ERRNO_NOERR;
    unsigned char      Address[4];
    unsigned char      WrData[8];
    unsigned char      offset = 0;
    const unsigned int MAX_BUF_LEN = 0x100;
    unsigned char      cr = 0;
    ...
    while(NumOfBytes)
    {
        ...
        *((unsigned int*)&Address) = ToBigEndiaULONG(dwAddr);
        //        dwAddr += offset;
        offset = 0;
...
int DownloadFW(const unsigned char * const pRomBin)
{
    ...
    do{
        ...
        if (0 == memcmp(CHAN_PATH,pRom->sDesc,sizeof(CHAN_PATH)-1)) {
			printk(KERN_INFO "[CNXT] sDesc = %s, sizeof CHAN_PATH = %d\n", pRom->sDesc, sizeof(CHAN_PATH));
			bIsRomVersion = 1;
        }
       ...
       // InitShowProgress(TotalLen);
        ...
        for( ;pRomData!=pRomDataEnd;)
        {
            ...
            pDataEnd= pData + FromBigEndiaULONG(pRomData->Length) - sizeof(unsigned int); 
        ...
        for( ;pRomData!=pRomDataEnd;)
        {
            ...
            BIF(CxWriteMemory(CurAddr,cbDataLen -sizeof(unsigned int)/*subtracts the address bytes*/ , pData, MEM_TYPE_RAM ));
            // The next RoMData position = current romData position + cbDataLen + sizeof( data len bytes)
            pRomData  =   (struct CX_CODEC_ROM_DATA *)((char*) pRomData + cbDataLen + sizeof(unsigned int));  
        ...
        for( ;pRomData!=pRomDataEnd;)
        {
            ...
            pData       = &pRomData->data[0];
            cbDataLen   = FromBigEndiaULONG(pRomData->Length) ;
            BIF(CxWriteMemory(CurAddr,cbDataLen -sizeof(unsigned int)/*subtracts the address bytes*/ , pData, MEM_TYPE_SPX ));
            // The next RoMData position = current romData position + cbDataLen + sizeof( data len bytes)
            pRomData  =   (struct CX_CODEC_ROM_DATA *)((char*) pRomData + cbDataLen + sizeof(unsigned int));  
